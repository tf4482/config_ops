...
def registry_path_from_config(peripherals: dict[str, Any]) -> str:
    registry_path = str(
        peripherals.get("registry_path", "")
    ).strip()

    if not registry_path:
        raise ValueError(
            "Configuration value "
            "'peripherals.registry_path' must be set"
        )
...
def devices_from_config(
    ...
    for name, value in peripherals.items():
        ...
        if not isinstance(value, dict):
            raise TypeError(
                f"Peripheral device '{name}' must be a table"
            )
        ...
        if not on_url:
            raise ValueError(
                f"Peripheral device '{name}' "
                "must define an 'on' URL"
            )

        if not off_url:
            raise ValueError(
                f"Peripheral device '{name}' "
                "must define an 'off' URL"
            )
    ...
    if not devices:
        raise ValueError(
            "No peripheral devices configured"
        )
...
def parse_arguments(
    ...
    if not selected_device_names:
        selected_device_names = list(
            devices.keys()
        )

    return (
        [devices[name] for name in selected_device_names],
        command,
    )
...
def main() -> None:
    ...
    registry_path = registry_path_from_config(
        peripherals
    )

    devices = devices_from_config(
        peripherals
    )
    ...
    selected_devices, command = parse_arguments(
        arguments,
        devices,
    )
